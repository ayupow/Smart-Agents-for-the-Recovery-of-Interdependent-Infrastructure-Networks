...
def network_assessment_of_closeness_centrality(global_json_path):
    """
    Determine recovery order based on closeness centrality and evaluate using population data.
    ...
    # Filter the closeness centrality for only the failed nodes
    failed_nodes_closeness = {node: closeness_centrality[node] for node in failed_nodes if node in closeness_centrality}

    # Sort failed nodes by Closeness Centrality (highest first)
    sorted_failed_nodes_by_closeness = sorted(failed_nodes_closeness, key=failed_nodes_closeness.get, reverse=True)

    recovery_order = sorted_failed_nodes_by_closeness  # Recovery order based solely on closeness centrality
    ...
    result = {
        'recovery_order': recovery_order,  # Recovery order based on closeness centrality
    ...
    output_json_path = 'network_assessment_of_closeness_centrality.json'

    with open(output_json_path, 'w') as outfile:
        json.dump(result, outfile, indent=4)

    # Update the Global_Data.json file with the recovery strategy result path
    file_paths['network_assessment_of_closeness_centrality'] = output_json_path
