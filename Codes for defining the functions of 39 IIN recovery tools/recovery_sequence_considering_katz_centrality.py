...
def recovery_strategy_of_katz_centrality(global_json_path):
    """
    Generate an interdependent infrastructure recovery strategy based on Katz centrality.
    ...
    # Read Katz centrality file
    with open(file_paths['facility_importance_using_katz_centrality'], 'r') as f:
    ...
    sorted_failed_edges = sorted(
        ...
        key=lambda x: (G.nodes[x[0]]['katz_centrality'] + G.nodes[x[1]]['katz_centrality']),
        reverse=True
    )
    ...
    # Output file path
    output_json_path = 'recovery_strategy_of_katz_centrality.json'
    ...
    file_paths['recovery_strategy_of_katz_centrality'] = output_json_path
