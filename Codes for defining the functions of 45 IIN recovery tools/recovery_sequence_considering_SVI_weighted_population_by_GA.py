...
def recovery_sequence_considering_SVI_weighted_population_by_GA(global_json_path):
    # Clean path
    ...
    population_data_path = global_data["population_SVI_weighted"]
    ...
    def calculate_population_affected(recovered_nodes):
        ...
        for area in affected_areas:
            for data in population_data:
                if data["Id"] == area:
                    affected_population += data["SVI_weighted_population"]
    ...
    output_json_path = 'recovery_sequence_considering_SVI_weighted_population_by_GA.json'
    with open(output_json_path, 'w') as f:
        json.dump(daily_recovery_order, f, indent=4)

    # Update global data output path
    global_data["recovery_sequence_considering_SVI_weighted_population_by_GA"] = output_json_path
    with open(global_json_path, 'w') as f:
        json.dump(global_data, f, indent=4)

    return "The path to recovery order result has been saved in global_data.json"

recovery_sequence_considering_SVI_weighted_population_by_GA("Global_Data.json")