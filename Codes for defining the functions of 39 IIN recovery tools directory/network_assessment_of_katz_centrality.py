...
def network_assessment_of_katz_centrality(global_json_path):
    """
    Determine recovery order based on Katz centrality and evaluate using population data.
    ...
    if not failed_nodes:
        ...
        # Filter the Katz centrality for only the failed nodes
        failed_nodes_katz = {node: katz_centrality[node] for node in failed_nodes if node in katz_centrality}

        # Sort failed nodes by Katz Centrality (highest first)
        sorted_failed_nodes_by_katz = sorted(failed_nodes_katz, key=failed_nodes_katz.get, reverse=True)

        recovery_order = sorted_failed_nodes_by_katz  # Recovery order based solely on Katz centrality
        ...
        result = {
            'recovery_order': recovery_order,  # Recovery order based on Katz centrality
        ...
        output_json_path = 'network_assessment_of_katz_centrality.json'

        with open(output_json_path, 'w') as outfile:
            json.dump(result, outfile, indent=4)

        # Update the Global_Data.json file with the recovery strategy result path
        file_paths['network_assessment_of_katz_centrality'] = output_json_path
