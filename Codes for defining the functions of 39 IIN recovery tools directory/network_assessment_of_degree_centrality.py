...
def network_assessment_of_degree_centrality(global_json_path):
    """
    Determine recovery order based on degree centrality and evaluate using population data.
    ...
    # Filter the degree centrality for only the failed nodes
    failed_nodes_degree = {node: degree_centrality[node] for node in failed_nodes if node in degree_centrality}

    # Sort failed nodes by Degree Centrality (highest first)
    sorted_failed_nodes_by_degree = sorted(failed_nodes_degree, key=failed_nodes_degree.get, reverse=True)

    recovery_order = sorted_failed_nodes_by_degree  # Recovery order based solely on degree centrality
    ...
    result = {
        'recovery_order': recovery_order,  # Recovery order based on degree centrality
    ...
    output_json_path = 'network_assessment_of_degree_centrality.json'

    with open(output_json_path, 'w') as outfile:
        json.dump(result, outfile, indent=4)

    # Update the Global_Data.json file with the recovery strategy result path
    file_paths['network_assessment_of_degree_centrality'] = output_json_path
