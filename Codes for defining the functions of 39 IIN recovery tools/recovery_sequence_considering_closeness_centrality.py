...
def recovery_strategy_of_closeness_centrality(global_json_path):
    ...
    # Read closeness centrality file
    with open(file_paths['facility_importance_using_closeness_centrality'], 'r') as f:
    ...
    sorted_failed_edges = sorted(
        ...
        key=lambda x: (G.nodes[x[0]]['closeness_centrality'] + G.nodes[x[1]]['closeness_centrality']),
        reverse=True
    )
    ...
    # Output file path
    output_json_path = 'recovery_strategy_of_closeness_centrality.json'
    ...
    file_paths['recovery_strategy_of_closeness_centrality'] = output_json_path
