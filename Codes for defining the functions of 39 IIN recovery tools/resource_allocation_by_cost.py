...
def resource_allocation_by_cost(global_json_path):
    ...
    def calculate_repair_cost(resource_allocation):
        total_cost = 0
        for rtype in resource_types:
            total_cost += resource_allocation[rtype] * resource_constraints['repair_teams'][rtype]['cost_per_day']
        return total_cost
    ...
    def fitness(order):
        ...
        for node in order:
            ...
            if node_data:
                cost = sum([node_data.get(rtype, 0) * resource_constraints['repair_teams'][rtype]['cost_per_day'] for rtype in resource_types])
    ...
    while best_order or node_progress:
        ...
        if resource_allocation_today:
            ...
            daily_cost = calculate_repair_cost({rtype: sum([alloc["resources_used"].get(rtype, 0) for alloc in resource_allocation_today]) for rtype in resource_types})
    ...
    output_json_path = 'resource_allocation_by_cost.json'
    with open(output_json_path, 'w') as f:
        json.dump(daily_recovery_order, f, indent=4)

    global_data["resource_allocation_by_cost"] = output_json_path
    with open(global_json_path, 'w') as f:
        json.dump(global_data, f, indent=4)

    return "The path to optimal recovery order result has been saved in global_data.json"
