...
def network_assessment_of_pagerank(global_json_path):
    """
    Determine recovery order based on PageRank centrality and evaluate using population data.
    ...
    # Filter the PageRank for only the failed nodes
    failed_nodes_pagerank = {node: pagerank[node] for node in failed_nodes if node in pagerank}

    # Sort failed nodes by PageRank (highest first)
    sorted_failed_nodes_by_pagerank = sorted(failed_nodes_pagerank, key=failed_nodes_pagerank.get, reverse=True)

    recovery_order = sorted_failed_nodes_by_pagerank  # Recovery order based solely on PageRank
    ...
    result = {
        'recovery_order': recovery_order,  # Recovery order based on PageRank
    ...
    output_json_path = 'network_assessment_of_pagerank.json'

    with open(output_json_path, 'w') as outfile:
        json.dump(result, outfile, indent=4)

    # Update the Global_Data.json file with the recovery strategy result path
    file_paths['network_assessment_of_pagerank'] = output_json_path
